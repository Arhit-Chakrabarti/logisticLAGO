...
#' @param sample.size a list of dimension equal to nstages providing the sample size (njk) in each center of each of the K stages
...
#' (see \code{\link{mc_lago_uc}} for multi-center LAGO with unequal center per stage and equal sample size per center)
#' @export
#'
#' @examples
...
mc_lago_uc.us <- function(x0, lower, upper, nstages, centers, beta.true, sample.size, icc, bcc, cost.vec, prob, intercept = TRUE, B = 100){
  ...
  if(length(sample.size) != nstages){
    stop("Sample size in each center in each stage should match the number of stages")
  }
  ...
  for(b in 1:B){
    ...
    for(k in 1:nstages){
      ...
      for(j in 1:centers[k]){
        ...
        actual_intervention[[j]] = jitter_my(as.vector(actual_intervention_center), n = sample.size[[k]], variation = icc) # Create the intervention data within each center per stage
      ...
      response <- rep(NA, sample.size[[k]] * centers[k])  # Initialize the vector of responses
      for(i in 1:(sample.size[[k]] * centers[k])){
